...
shinyServer(function(input,output){
    ...
    output$histo <- renderPlot({
        dist <- input$dist
        n <- input$n
        hist(dat()[[1]],main="Histogram of selected distribution",xlab="Observations",col="skyblue",cex.axis=1.2,cex.lab=1.2,prob=T)
    ...
    output$dens <- renderPlot({
        dist <- input$dist
        n <- input$n
        plot(density(dat()[[1]]),main="Density plot of selected distribution",xlab="Observations",col = 'darkblue',cex.axis=1.2,cex.lab=1.2)
        if(input$density) lines(density(dat()[[1]],adjust=input$bw),lwd=2)
    })
    ...
    output$boxp <- renderPlot({
        dist <- input$dist
        n <- input$n
        boxplot(dat()[[1]],main="Box plot of selected distribution",xlab="Observations",col = 'skyblue')
    })
